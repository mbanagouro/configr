...
public sealed class RavenConfigStoreTests
{
    private async Task<RavenDbConfigStore> CreateStoreAsync()
    {
        ...
        var options = Options.Create(new RavenDbConfigStoreOptions
        {
            ...
            KeyPrefix = "configr-config-store-tests"
        });

        return new RavenDbConfigStore(documentStore, options);
    }
